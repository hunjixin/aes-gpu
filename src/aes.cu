...
__device__ void printBytes(BYTE b[], int len)
{
    int i;
    for (i = 0; i < len; i++) {
       printf("%02x", b[i]);
...
extern "C" __global__ void AES_Encrypt(int32_t keyLen, int32_t block_number,  aes_block aes_block_array[], BYTE key[5])
{
...
printBytes(key, 10);
printf("\nblock \n");
printBytes(aes_block_array[0].block, 16);
printf("\n");
...
__global__ void AES_Decrypt(aes_block aes_block_array[], BYTE key[], int32_t keyLen, int32_t block_number)
{
    int global_thread_index = blockDim.x * blockIdx.x + threadIdx.x;
    __shared__ BYTE AES_ShiftRowTab[16];
    __shared__ BYTE AES_Sbox[256];
